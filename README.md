...
## 📦 Servicios Incluidos
1. [cite_start]**SQL Server 2022:** Base de datos relacional core para transacciones[cite: 65].
2. [cite_start]**MongoDB:** Almacén de eventos y auditoría[cite: 66].
3. [cite_start]**Mock Service (Wiremock):** Simulación de API externa de catálogo de servicios[cite: 67].
4. [cite_start]**Incident API:** Backend en .NET 8[cite: 68].
5. [cite_start]**Incident Web:** Frontend web[cite: 69].

## 🚀 Ejecución del Proyecto (Pasos Exactos)

[cite_start]Para cumplir con el checklist de aceptación[cite: 92], ejecute el sistema con un solo comando.

### 1. Layout de Carpetas Requerido
[cite_start]Se asume este layout al clonar los 3 repositorios:
```text
/workspace
  ├── /incident-api
  ├── /incident-web
